...
class Store:
    ...
    def store_route_mon(self, msg, src):
        ...
        # save raw BGP packet
        sql = """INSERT INTO raw_log (ts, bmp_src, bgp_src, bgp_data) VALUES (%s, %s, %s, %s)"""
        self.curs.execute(sql, (msg.time, src.host, msg.peer_address, psycopg2.Binary(msg.raw_payload)))

        #
        # update adj_rib_in table

        # nlri
        find_path = """SELECT id FROM adj_rib_in
            WHERE
                bmp_src = %s
                AND neighbor_addr = %s
                AND prefix = %s
                AND contains(valid, CURRENT_TIMESTAMP)"""
        invalidate_prev = """UPDATE adj_rib_in SET valid = ( '[' || first(valid) || ', ' || %s || ' )' )::period WHERE id = %s"""
        insert_path = """INSERT INTO adj_rib_in
            (
                valid,
                bmp_src,
                neighbor_addr,
        ...
        for pref in msg.update.nlri:
            ...
            self.curs.execute(find_path, (src.host, msg.peer_address, pref.prefix))
            self.time_select += time.time() - start_select
            if self.curs.rowcount > 0:
                row = self.curs.fetchone()
                start_update = time.time()
                try:
                    self.curs.execute(invalidate_prev, (msg.time, row[0]))
            ...
            self.curs.execute(insert_path, args)
        ...
        if self.nmsg % 10000 == 0:
            now = time.time()
            self._logger.debug('nmsg %d %.1f msg/s; npref %d %.1f pref/s select: %f update: %f insert: %f' % (
                self.nmsg,
                10000/(now - self.last_ts),
                self.npref,
                (self.npref - self.last_npref)/(now - self.last_ts),
                self.time_select,
                self.time_update,
                self.time_insert
            ))
            self.last_ts = now
            self.last_npref = self.npref
